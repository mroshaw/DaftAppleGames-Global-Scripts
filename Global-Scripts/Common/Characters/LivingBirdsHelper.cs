...
namespace DaftAppleGames.Common.Characters
{
    public class LivingBirdsHelper : MonoBehaviour
    {
        ...
        // Private fields
        private lb_BirdController[] _allBirdControllers;
        ...
        private void Start()
        {
            _allBirdControllers = FindObjectsByType<lb_BirdController>(FindObjectsSortMode.None);
        }
        #endregion
        ...
        {
            foreach (lb_BirdController controller in _allBirdControllers)
            {
                if (isEnabled)
                {
